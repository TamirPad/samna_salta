...
from src.infrastructure.container.dependency_injection import (
    DependencyContainer,
    get_container,
...
try:
    from src.infrastructure.deployment.health_checks import (
...
try:
    from src.infrastructure.utilities.config_validator import (
        ConfigValidator,
...
try:
    from src.infrastructure.utilities.security import (
        SecurityUtils,
...
@pytest.mark.skipif(
    not HEALTH_CHECKS_AVAILABLE, reason="Health checks module not available"
)
class TestHealthCheckService:
    ...
    async def test_run_health_checks_with_checkers(self, health_service):
        ...
        checker.check = AsyncMock(
            return_value={"status": "healthy", "details": "All good"}
        )
    ...
    async def test_database_health_checker(self):
        ...
        with patch(
            "src.infrastructure.deployment.health_checks.get_session"
        ) as mock_get_session:
...
@pytest.mark.skipif(
    not CONFIG_VALIDATOR_AVAILABLE, reason="Config validator module not available"
)
class TestConfigValidator:
    ...
    def test_validate_security_config_weak_key(self):
        ...
        config = {"SECRET_KEY": "weak", "RATE_LIMIT_PER_MINUTE": 60}
...
@pytest.mark.skipif(
    not LOGGING_CONFIG_AVAILABLE, reason="Logging config module not available"
)
class TestLoggingConfig:
    ...
    def test_setup_logging_basic(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            result = setup_logging(
                log_level="INFO", log_file=log_file, enable_json=False
            )
    ...
    def test_logging_config_invalid_file_path(self):
        ...
        result = setup_logging(log_file="/invalid/path/that/does/not/exist/test.log")
...
@pytest.mark.skipif(
    not SECURITY_UTILS_AVAILABLE, reason="Security utils module not available"
)
