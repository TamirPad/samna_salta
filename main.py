        ...
        while retry_count < max_retries:
            try:
                ...
                if "Conflict: terminated by other getUpdates request" in str(e):
                    logger.warning(f"Bot conflict detected on attempt {retry_count}. Waiting 30 seconds before retry...")
                    if retry_count < max_retries:
                        import time
                        time.sleep(30)  # Wait 30 seconds before retrying
                    else:
                        logger.error("Max retries reached. Bot conflict could not be resolved.")
                        raise
                else:
