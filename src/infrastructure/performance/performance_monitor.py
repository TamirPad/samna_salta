...
class PerformanceState:
    ...
    metrics_history: deque = field(default_factory=lambda: deque(maxlen=10000))
...
class PerformanceMonitor:
    ...
    def _collect_application_metrics(self):
        ...
        if self.state.counters.request_count > 0:
            error_rate = (
                self.state.counters.error_count / self.state.counters.request_count
            ) * 100
            self._record_metric(
                RecordMetric(
                    name="error_rate",
        ...
        if self.state.counters.response_times:
            ...
            p95_response_time = statistics.quantiles(
                self.state.counters.response_times, n=20
            )[18]
            self._record_metric(
                RecordMetric(
        ...
        for metric_name, metric in self.state.current_metrics.items():
            if metric_name in self.state.thresholds:
                ...
                if metric.value >= thresholds.get("critical", float("inf")):
                    self._create_alert(
                        metric_name, thresholds["critical"], metric.value, "critical"
                    )
                elif metric.value >= thresholds.get("warning", float("inf")):
                    self._create_alert(
                        metric_name, thresholds["warning"], metric.value, "warning"
                    )

    def _create_alert(
        self, metric_name: str, threshold: float, current_value: float, severity: str
    ):
        """Create a performance alert"""
        # Avoid duplicate alerts for the same metric
        recent_alerts = [
            alert
            for alert in self.state.alerts[-10:]
    ...
    def _cleanup_old_data(self):
        ...
        self.state.alerts = [
            alert for alert in self.state.alerts if alert.timestamp > cutoff_time
        ]
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "avg_response_time": (
                statistics.mean(self.state.counters.response_times)
                if self.state.counters.response_times
                else 0
            ),
            "avg_db_query_time": (
                statistics.mean(self.state.counters.db_query_times)
                if self.state.counters.db_query_times
                else 0
            ),
            "avg_cache_hit_rate": (
                statistics.mean(self.state.counters.cache_hit_rates)
                if self.state.counters.cache_hit_rates
                else 0
            ),
            "current_cpu_usage": (
                self.state.current_metrics["cpu_usage"].value
                if "cpu_usage" in self.state.current_metrics
                else 0
            ),
            "current_memory_usage": (
                self.state.current_metrics["memory_usage"].value
                if "memory_usage" in self.state.current_metrics
                else 0
            ),
    ...
    def _calculate_trends(self) -> Dict[str, str]:
        ...
        for metric_name in ["avg_response_time", "error_rate", "cpu_usage"]:
            history = [m.value for m in self.get_metric_history(metric_name, hours=1)]
            if len(history) > 10:
    ...
    def _generate_recommendations(self) -> List[OptimizationRecommendation]:
        ...
        if (
            ...
            > self.state.thresholds["response_time"]["warning"]
        ):
        ...
        # High DB query time
        if (
            summary["avg_db_query_time"]
            > self.state.thresholds["db_query_time"]["warning"]
        ):
            recommendations.append(
                OptimizationRecommendation(
                    category="Database",
        ...
        if (
            ...
            < self.state.thresholds["cache_hit_rate"]["warning"]
        ):
            recommendations.append(
                OptimizationRecommendation(
                    category="Caching",
        ...
        if (
            summary["current_cpu_usage"] > self.state.thresholds["cpu_usage"]["warning"]
            or summary["current_memory_usage"]
            > self.state.thresholds["memory_usage"]["warning"]
        ):
            recommendations.append(
                OptimizationRecommendation(
    ...
    def _calculate_system_health(self) -> str:
        ...
        if (
            summary["avg_response_time"]
            > self.state.thresholds["response_time"]["critical"]
        ):
            score -= 30
        ...
        return json.dumps(
            {
                "summary": self.get_performance_summary(),
                "current_metrics": {
                    k: v.__dict__ for k, v in self.state.current_metrics.items()
                },
                "recent_alerts": [a.__dict__ for a in self.get_recent_alerts(1)],
                "recommendations": [
                    r.__dict__ for r in self._generate_recommendations()
                ],
            },
            default=str,
            indent=4,
