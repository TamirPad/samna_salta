...
from ...application.use_cases.customer_registration_use_case import CustomerRegistrationUseCase
...
from ...application.use_cases.order_status_management_use_case import OrderStatusManagementUseCase
...
class DependencyContainer:
    ...
    def _register_services(self):
        ...
        if self._bot:
            ...
            self._instances['customer_notification_service'] = CustomerNotificationService(
                bot=self._bot,
                customer_repository=self.get_customer_repository()
            )
            ...
            self._logger.warning("Bot not provided, notification services not registered")
    ...
    def _register_use_cases(self):
        ...
        if 'admin_notification_service' in self._instances:
            order_creation_kwargs['admin_notification_service'] = self.get_admin_notification_service()
        
        self._instances['order_creation_use_case'] = OrderCreationUseCase(**order_creation_kwargs)
        ...
        if 'admin_notification_service' in self._instances:
            status_management_kwargs['admin_notification_service'] = self.get_admin_notification_service()
        if 'customer_notification_service' in self._instances:
            status_management_kwargs['customer_notification_service'] = self.get_customer_notification_service()
        
        self._instances['order_status_management_use_case'] = OrderStatusManagementUseCase(**status_management_kwargs)
