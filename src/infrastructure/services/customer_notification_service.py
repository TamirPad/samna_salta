...
class CustomerNotificationService:
    ...
    async def notify_order_status_update(
        ...
        self._logger.info(
            f"📨 SENDING CUSTOMER NOTIFICATION: Order #{order_info.order_number}"
        )
        ...
        try:
            ...
            if not customer:
                self._logger.warning(
                    f"⚠️ CUSTOMER NOT FOUND: Phone {order_info.customer_phone}"
                )
                return
            ...
            try:
                ...
                self._logger.info(
                    f"✅ CUSTOMER NOTIFICATION SENT: {customer.telegram_id.value}"
                )
            except TelegramError as e:
                self._logger.error(
                    f"❌ CUSTOMER NOTIFICATION FAILED: {customer.telegram_id.value}, Error: {e}"
                )
    ...
    async def notify_order_ready_for_pickup(self, order_info: OrderInfo) -> None:
        ...
        self._logger.info(
            f"📦 SENDING PICKUP NOTIFICATION: Order #{order_info.order_number}"
        )
        ...
        try:
            ...
            try:
                ...
                self._logger.info(
                    f"✅ PICKUP NOTIFICATION SENT: {customer.telegram_id.value}"
                )
            except TelegramError as e:
                self._logger.error(
                    f"❌ PICKUP NOTIFICATION FAILED: {customer.telegram_id.value}, Error: {e}"
                )
