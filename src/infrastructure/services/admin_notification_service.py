...
class AdminNotificationService:
    ...
    def __init__(self, bot: Bot, customer_repository: CustomerRepository):
        ...
        # Log initialization
        self._logger.info(f"🏗️ ADMIN NOTIFICATION SERVICE INITIALIZED")
    ...
    async def notify_new_order(self, order_info: OrderInfo) -> None:
        ...
        self._logger.info(f"📨 ===== ADMIN NOTIFICATION STARTED =====")
        self._logger.info(f"📨 SENDING ADMIN NOTIFICATION: Order #{order_info.order_number}")
        self._logger.info(f"📨 Order Details: Customer={order_info.customer_name}, Total=₪{order_info.total:.2f}")
        
        try:
            # Get admin users
            self._logger.info(f"👑 STEP 1: Finding admin users...")
            admin_users = await self._get_admin_users()
            if not admin_users:
                self._logger.warning("⚠️ NO ADMIN USERS FOUND - Cannot send notifications!")
                return
            
            self._logger.info(f"✅ FOUND {len(admin_users)} ADMIN USERS: {admin_users}")
            
            # Create notification message
            self._logger.info(f"📝 STEP 2: Creating notification message...")
            ...
            # Send to each admin
            self._logger.info(f"📤 STEP 3: Sending notifications to admins...")
            successful_sends = 0
            failed_sends = 0
            ...
            for admin_telegram_id in admin_users:
                ...
                try:
                    ...
                    self._logger.info(f"✅ NOTIFICATION SENT SUCCESSFULLY: Admin {admin_telegram_id}, Message ID: {result.message_id}")
                    successful_sends += 1
                except TelegramError as e:
                    self._logger.error(f"❌ TELEGRAM ERROR: Admin {admin_telegram_id}, Error: {e}")
                    failed_sends += 1
                except Exception as e:
                    self._logger.error(f"💥 NOTIFICATION FAILED: Admin {admin_telegram_id}, Error: {e}", exc_info=True)
                    failed_sends += 1
            
            self._logger.info(f"📊 NOTIFICATION SUMMARY:")
            self._logger.info(f"  ✅ Successful: {successful_sends}")
            self._logger.info(f"  ❌ Failed: {failed_sends}")
            self._logger.info(f"📨 ===== ADMIN NOTIFICATION COMPLETED =====")
    ...
    async def notify_order_status_update(self, order_info: OrderInfo, old_status: str) -> None:
        ...
        self._logger.info(f"📨 SENDING STATUS UPDATE NOTIFICATION: Order #{order_info.order_number}")
        ...
        try:
            ...
            for admin_telegram_id in admin_users:
                try:
                    ...
                    self._logger.error(f"❌ STATUS NOTIFICATION FAILED: Admin {admin_telegram_id}, Error: {e}")
    ...
    async def _get_admin_users(self) -> List[int]:
        ...
        self._logger.info(f"👑 SEARCHING FOR ADMIN USERS...")
        try:
            ...
            for i, customer in enumerate(customers):
                self._logger.info(f"👤 Customer {i+1}: ID={customer.id.value}, Name={customer.full_name.value}, TelegramID={customer.telegram_id.value}, Admin={customer.is_admin}")
                
                if customer.is_admin:
                    admin_users.append(customer.telegram_id.value)
                    self._logger.info(f"  👑 ADMIN FOUND: {customer.full_name.value} (TelegramID: {customer.telegram_id.value})")
            
            self._logger.info(f"👑 ADMIN SEARCH COMPLETE: Found {len(admin_users)} admin users")
            if admin_users:
                self._logger.info(f"👑 ADMIN TELEGRAM IDS: {admin_users}")
            else:
                self._logger.warning(f"⚠️ NO ADMIN USERS FOUND!")
