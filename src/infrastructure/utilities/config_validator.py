            ...
            if response.status_code != 200:
                ...
                if bot_info.get("ok"):
                    self.warnings.append(
                        f"Bot validated: @{bot_info['result']['username']}"
                    )
                else:
    ...
    def _validate_database_configuration(self):
        ...
        if self.config.database_url.startswith("sqlite:"):  # pylint: disable=no-member
            ...
            if not Path(db_path).exists():
                self.warnings.append(
                    f"Database file does not exist (will be created): {db_path}"
                )
    ...
    def _log_validation_results(self):
        ...
        if self.errors:
            logger.error(
                ...
                extra={"errors": self.errors, "warnings": self.warnings},
            )
        elif self.warnings:
            logger.warning(
...
def validate_production_readiness() -> bool:
    ...
    if is_valid and not report["warnings"]:
        ...
        print(
            "✅ Configuration validation PASSED with warnings - Review before production"
        )
