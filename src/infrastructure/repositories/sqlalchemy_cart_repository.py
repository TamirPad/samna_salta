...
from ..database.models import Cart as SQLCart
...
class SQLAlchemyCartRepository(CartRepository):
    ...
    async def get_cart_items(self, telegram_id: TelegramId) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            try:
                ...
                if not cart:
                    self._logger.info(
                        f"📭 NO CART: User {telegram_id.value} has no cart"
                    )
                    return None

                self._logger.info(
                    f"📦 CART FOUND: User {telegram_id.value} has {len(cart.items or [])} raw items"
                )
                ...
                for i, item in enumerate(cart.items or []):
                    ...
                    self._logger.debug(
                        f"📋 PROCESSING ITEM {i}: product_id={product_id}, item={item}"
                    )
                    ...
                    if product:
                        ...
                        self._logger.warning(
                            f"⚠️ PRODUCT NOT FOUND: product_id={product_id} for cart item"
                        )
                ...
                self._logger.info(
                    f"📊 CART RESULT: {len(detailed_items)} valid items, delivery={cart.delivery_method}"
                )
                return result
            ...
            self._logger.error(
                f"💥 DATABASE ERROR getting cart items for user {telegram_id.value}: {e}"
            )
            raise
        except Exception as e:
            self._logger.error(
                f"💥 UNEXPECTED ERROR getting cart items for user {telegram_id.value}: {e}"
            )
            raise
    ...
    async def add_item(
        ...
        """Add item to cart"""
        self._logger.info(
            f"➕ ADD ITEM: User {telegram_id.value}, Product {product_id.value}, Qty {quantity}, Options {options}"
        )
        ...
        try:
            ...
            try:
                ...
                if not cart:
                    self._logger.info(
                        f"🆕 CREATING CART: New cart for user {telegram_id.value}"
                    )
                    cart = SQLCart(telegram_id=telegram_id.value, items=[])
                    session.add(cart)
                else:
                    self._logger.info(
                        f"📦 EXISTING CART: Found cart with {len(cart.items or [])} items"
                    )
                ...
                for i, item in enumerate(items):
                    if (
                        ...
                        existing_item = i
                        self._logger.info(
                            f"🔄 UPDATING EXISTING: Item {i} already exists, updating quantity"
                        )
                        break
                ...
                if existing_item is not None:
                    ...
                    self._logger.info(
                        f"📈 QUANTITY UPDATE: {old_qty} → {items[existing_item]['quantity']}"
                    )
                else:
                    # Add new item
                    new_item = {
                ...
                self._logger.info(
                    f"✅ CART UPDATED: User {telegram_id.value} now has {actual_items} items (verified)"
                )
            ...
            self._logger.error(
                f"💥 DATABASE ERROR adding item to cart: User {telegram_id.value}, Product {product_id.value}, Error: {e}"
            )
            raise
        except Exception as e:
            self._logger.error(
                f"💥 UNEXPECTED ERROR adding item to cart: User {telegram_id.value}, Product {product_id.value}, Error: {e}"
            )
            raise
    ...
    async def update_cart(
        ...
        """Update entire cart"""
        self._logger.info(
            f"🔄 UPDATE CART: User {telegram_id.value}, {len(items)} items, delivery={delivery_method}"
        )
        ...
        try:
            ...
            try:
                ...
                self._logger.info(
                    f"💾 COMMITTING: {len(items)} items, delivery={cart.delivery_method}"
                )
                session.commit()
            ...
            self._logger.error(
                f"💥 DATABASE ERROR updating cart: User {telegram_id.value}, Error: {e}"
            )
            raise
        except Exception as e:
            self._logger.error(
                f"💥 UNEXPECTED ERROR updating cart: User {telegram_id.value}, Error: {e}"
            )
            raise
    ...
    async def clear_cart(self, telegram_id: TelegramId) -> bool:
        ...
        try:
            ...
            try:
                ...
                if cart:
                    self._logger.info(
                        f"📦 FOUND CART: User {telegram_id.value} had {len(cart.items or [])} items"
                    )
                    ...
                    self._logger.info(
                        f"💾 COMMITTING: Cleared cart for user {telegram_id.value}"
                    )
                    session.commit()
                    ...
                    actual_items = len(cart.items or [])
                    self._logger.info(
                        f"✅ CART CLEARED: User {telegram_id.value} now has {actual_items} items (verified)"
                    )
                else:
                    self._logger.info(
                        f"📭 NO CART: User {telegram_id.value} had no cart to clear"
                    )
            ...
            self._logger.error(
                f"💥 DATABASE ERROR clearing cart: User {telegram_id.value}, Error: {e}"
            )
            raise
        except Exception as e:
            self._logger.error(
                f"💥 UNEXPECTED ERROR clearing cart: User {telegram_id.value}, Error: {e}"
            )
            raise
