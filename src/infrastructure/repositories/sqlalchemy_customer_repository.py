...
class SQLAlchemyCustomerRepository(CustomerRepository):
    ...
    async def find_by_telegram_id(
        ...
        try:
            ...
            self._logger.error(
                f"Database error finding customer by telegram ID {telegram_id.value}: {e}"
            )
            raise
        except Exception as e:
            self._logger.error(
                f"Unexpected error finding customer by telegram ID {telegram_id.value}: {e}"
            )
            raise
    ...
    async def find_by_phone_number(
        ...
        try:
            ...
            self._logger.error(
                f"Database error finding customer by phone {phone_number.value}: {e}"
            )
            raise
        except Exception as e:
            self._logger.error(
                f"Unexpected error finding customer by phone {phone_number.value}: {e}"
            )
            raise
        ...
        try:
            ...
            try:
                if customer.id is None:
                    ...
                    if not sql_customer:
                        raise ValueError(
                            f"Customer with ID {customer.id.value} not found"
                        )
    ...
    async def delete(self, customer_id: CustomerId) -> bool:
        ...
        try:
            ...
            self._logger.error(
                f"Database error deleting customer {customer_id.value}: {e}"
            )
            raise
        except Exception as e:
            self._logger.error(
                f"Unexpected error deleting customer {customer_id.value}: {e}"
            )
            raise
    ...
    async def find_by_id(self, customer_id: CustomerId) -> Optional[DomainCustomer]:
        ...
        try:
            ...
            self._logger.error(
                f"Database error finding customer by ID {customer_id.value}: {e}"
            )
            raise
        except Exception as e:
            self._logger.error(
                f"Unexpected error finding customer by ID {customer_id.value}: {e}"
            )
            raise
    ...
    async def exists_by_telegram_id(self, telegram_id: TelegramId) -> bool:
        ...
        try:
            ...
            self._logger.error(
                f"Database error checking telegram ID {telegram_id.value}: {e}"
            )
            raise
        except Exception as e:
            self._logger.error(
                f"Unexpected error checking telegram ID {telegram_id.value}: {e}"
            )
            raise
    ...
    async def exists_by_phone_number(self, phone_number: PhoneNumber) -> bool:
        ...
        try:
            ...
            self._logger.error(
                f"Database error checking phone number {phone_number.value}: {e}"
            )
            raise
        except Exception as e:
            self._logger.error(
                f"Unexpected error checking phone number {phone_number.value}: {e}"
            )
            raise
