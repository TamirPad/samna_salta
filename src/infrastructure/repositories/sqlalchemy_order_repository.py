...
from ..database.models import Order, OrderItem, Customer
...
class SQLAlchemyOrderRepository(OrderRepository):
    ...
    async def get_orders_by_customer(self, customer_id: CustomerId) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            try:
                orders = session.query(Order).filter(
                    Order.customer_id == customer_id.value
                ...
                self._logger.info(f"✅ FOUND {len(result)} ORDERS for customer {customer_id.value}")
                return result
    ...
    async def get_orders_by_telegram_id(self, telegram_id: TelegramId) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            try:
                ...
                customer = session.query(Customer).filter(
                    Customer.telegram_id == telegram_id.value
                ).first()
                
                if not customer:
                    self._logger.info(f"📭 CUSTOMER NOT FOUND: Telegram ID {telegram_id.value}")
                    return []
    ...
    async def update_order_status(self, order_id: OrderId, status: str) -> bool:
        ...
        try:
            ...
            try:
                ...
                self._logger.info(f"✅ STATUS UPDATED: Order #{order.order_number} {old_status} -> {status}")
                return True
    ...
    async def get_all_orders(self, limit: int = 100, status: Optional[str] = None) -> List[Dict[str, Any]]:
