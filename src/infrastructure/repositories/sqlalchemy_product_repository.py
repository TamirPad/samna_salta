...
class SQLAlchemyProductRepository(ProductRepository):
    ...
    async def find_by_id(self, product_id: ProductId) -> Optional[SQLProduct]:
        ...
        try:
            ...
            self._logger.error(
                f"Database error finding product by ID {product_id.value}: {e}"
            )
            raise
        except Exception as e:
            self._logger.error(
                f"Unexpected error finding product by ID {product_id.value}: {e}"
            )
            raise
    ...
    async def find_by_name(self, name: ProductName) -> Optional[SQLProduct]:
        ...
        try:
            ...
            try:
                return (
                    session.query(SQLProduct)
                    .filter(SQLProduct.name == name.value)
                    .first()
                )
            finally:
                session.close()
            ...
            self._logger.error(
                f"Database error finding product by name {name.value}: {e}"
            )
            raise
        except Exception as e:
            self._logger.error(
                f"Unexpected error finding product by name {name.value}: {e}"
            )
            raise
    ...
    async def find_by_category(self, category: str) -> List[SQLProduct]:
        ...
        try:
            ...
            self._logger.error(
                f"Database error finding products by category {category}: {e}"
            )
            raise
        except Exception as e:
            self._logger.error(
                f"Unexpected error finding products by category {category}: {e}"
            )
            raise
    ...
    async def delete(self, product_id: ProductId) -> bool:
        ...
        try:
            ...
            self._logger.error(
                f"Database error deleting product {product_id.value}: {e}"
            )
            raise
        except Exception as e:
            self._logger.error(
                f"Unexpected error deleting product {product_id.value}: {e}"
            )
            raise
