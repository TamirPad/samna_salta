...
from src.infrastructure.utilities.exceptions import error_handler, BusinessLogicError, OrderNotFoundError
...
class AdminHandler:
    ...
    async def _show_admin_dashboard(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
...
"""
            ...
            keyboard = [
                [
                    ...
                    InlineKeyboardButton("🔄 Active Orders", callback_data="admin_active_orders")
                ],
                [
                    InlineKeyboardButton("📋 All Orders", callback_data="admin_all_orders"),
                    InlineKeyboardButton("🔄 Update Status", callback_data="admin_update_status")
                ],
                [
            ...
            await update.effective_message.reply_text("Error loading dashboard. Please try again.")
...
"""
                ...
                for order in orders[:10]:
                    status_emoji = {'confirmed': '✅', 'preparing': '👨‍🍳', 'ready': '🛍️'}.get(order.status, '📋')
                    order_summary = f"{status_emoji} #{order.order_number} - {order.customer_name}"
                    keyboard.append([InlineKeyboardButton(
                        order_summary,
                        callback_data=f"admin_order_{order.order_id}"
                    )])
...
"""
    ...
    async def _show_order_details(self, query, order_id: int) -> None:
        ...
        try:
            ...
            delivery_emoji = '🚚' if order_data.get('delivery_method') == 'delivery' else '🏪'
...
"""
    ...
    async def _update_order_status(self, query, order_id: int, new_status: str, admin_telegram_id: int) -> None:
        """Update order status"""
        try:
            order_status_use_case = self._container.get_order_status_management_use_case()
            ...
            await query.message.reply_text("Error updating order status. Please try again.")
    
    async def _start_status_update(self, query, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    async def _is_admin_user(self, telegram_id: int) -> bool:
        ...
        try:
            ...
            customer = await customer_repository.find_by_telegram_id(TelegramId(telegram_id))
