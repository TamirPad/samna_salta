...
from src.infrastructure.utilities.exceptions import error_handler, BusinessLogicError
from src.application.dtos.cart_dtos import CartItemInfo, GetCartResponse, AddToCartRequest
from src.application.dtos.order_dtos import CreateOrderRequest
from src.presentation.telegram_bot.keyboards.menu import get_cart_keyboard, get_main_menu_keyboard
...
class CartHandler:
    ...
    async def handle_add_to_cart(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        try:
            ...
            if not product_info:
                self._logger.error(f"❌ INVALID CALLBACK: Could not parse {callback_data}")
            ...
            if response.success:
                ...
                self._logger.info(f"✅ ADD SUCCESS: {product_info['display_name']} added. Cart: {item_count} items, ₪{cart_total:.2f}")
            ...
            self._logger.error(f"💥 ADD TO CART ERROR: User {user_id}, Callback: {callback_data}, Error: {e}", exc_info=True)
            await query.edit_message_text(
    ...
    async def handle_view_cart(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    async def handle_send_order(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        try:
            ...
            self._logger.info(f"📝 STEP 1: Getting order creation use case...")
            ...
            if not order_creation_use_case:
                ...
                await query.edit_message_text("❌ Order system not available. Please try again later.")
                return
            self._logger.info(f"✅ STEP 1: Order creation use case obtained")
            
            # STEP 2: Check cart contents first
            self._logger.info(f"📝 STEP 2: Checking cart contents...")
            cart_use_case = self._container.get_cart_management_use_case()
            cart_response = await cart_use_case.get_cart(user_id)
            
            if not cart_response.success:
                self._logger.error(f"❌ STEP 2: Cart check failed: {cart_response.error_message}")
                await query.edit_message_text(
            ...
            if not cart_response.cart_summary or not cart_response.cart_summary.items:
                self._logger.error(f"❌ STEP 2: Cart is empty!")
                await query.edit_message_text(
            ...
            self._logger.info(f"✅ STEP 2: Cart has {len(cart_response.cart_summary.items)} items")
            
            # STEP 3: Create order request
            self._logger.info(f"📝 STEP 3: Creating order request...")
            cart_summary = cart_response.cart_summary
            ...
            self._logger.info(f"✅ STEP 3: Order request created - Method: {order_request.delivery_method}")
            
            # STEP 4: Create the order
            self._logger.info(f"📝 STEP 4: Creating order via use case...")
            order_response = await order_creation_use_case.create_order(order_request)
            
            if not order_response.success:
                self._logger.error(f"❌ STEP 4: Order creation failed: {order_response.error_message}")
                await query.edit_message_text(
            ...
            self._logger.info(f"✅ STEP 4: Order created successfully - Order #{order_response.order_info.order_number}")
            
            # STEP 5: Check admin notification service
            self._logger.info(f"📝 STEP 5: Checking admin notification service...")
            admin_service = self._container.get_admin_notification_service()
            if admin_service:
                self._logger.info(f"✅ STEP 5: Admin notification service is available")
            else:
                self._logger.warning(f"⚠️ STEP 5: Admin notification service NOT available")
            
            # STEP 6: Display success message
            self._logger.info(f"📝 STEP 6: Displaying success message...")
            ...
            self._logger.info(f"🎉 ORDER COMPLETED: Order #{order_response.order_info.order_number} for User {user_id}")
            
        except Exception as e:
            self._logger.error(f"💥 SEND ORDER ERROR: User {user_id}, Error: {e}", exc_info=True)
            await query.edit_message_text(
    ...
    def _format_order_confirmation(self, order_info) -> str:
        ...
        self._logger.info(f"📄 FORMATTING ORDER CONFIRMATION: Order #{order_info.order_number}")
...
"""
    ...
    async def _display_cart_contents(self, query, cart_response: GetCartResponse) -> None:
...
"""
            ...
            delivery_charge = 5.0 if cart_response.delivery_method == 'delivery' else 0.0
...
<b>Items:</b>"""
            ...
            delivery_method_display = (cart_response.delivery_method or 'pickup').title()
            message += f"""
...
def register_cart_handlers(application):
    ...
    application.add_handler(CallbackQueryHandler(
        cart_handler.handle_send_order, 
        pattern="^cart_send_order$"
    ))
    ...
    application.add_handler(CallbackQueryHandler(
        cart_handler.handle_add_to_cart,
        pattern="^(kubaneh_|samneh_|red_bisbas_)"
    ))
