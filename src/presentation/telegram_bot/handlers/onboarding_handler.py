...
from ..states import ONBOARDING_NAME, ONBOARDING_PHONE, ONBOARDING_DELIVERY_METHOD, ONBOARDING_DELIVERY_ADDRESS, END
...
from ....application.use_cases.customer_registration_use_case import (
    CustomerRegistrationRequest, CustomerRegistrationResponse
)
from ....infrastructure.utilities.exceptions import (
    ValidationError, BusinessLogicError, error_handler
)
...
class OnboardingHandler:
    ...
    def __init__(self):
        ...
        self._customer_registration_use_case = self._container.get_customer_registration_use_case()
    ...
    async def start_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
        ...
        self._logger.info(f"Start command received from user {user.id} ({user.username})")
        
        try:
            # Check if user is already registered using use case
            existing_customer = await self._customer_registration_use_case.find_customer_by_telegram_id(user.id)
            
            if existing_customer:
                # Welcome back existing customer
                welcome_message = f"Hi {existing_customer.full_name.value}, great to see you again! 🎉"
            ...
            await self._send_error_message(update, "Sorry, there was an error. Please try again with /start")
            return END
    ...
    async def handle_phone(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
        ...
        try:
            ...
            if not full_name:
                await self._send_error_message(update, "Session expired. Please start again with /start")
                return END
            ...
            response = await self._customer_registration_use_case.execute(registration_request)
    ...
    async def handle_delivery_method(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
        ...
        try:
            ...
            await self._send_error_message(update.callback_query, "Please try again with /start")
            return END
    
    async def handle_delivery_address(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
