...
class OrderAnalyticsUseCase:
    ...
    async def get_daily_summary(self, date: Optional[datetime] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            today_orders = [
                ...
                order['created_at'].date() == date.date()
            ]
            ...
            # Status breakdown
            status_counts = Counter(order.get('status', 'pending') for order in today_orders)
            
            # Delivery method breakdown
            delivery_counts = Counter(order.get('delivery_method', 'pickup') for order in today_orders)
            ...
            self._logger.info(f"📈 DAILY SUMMARY: {total_orders} orders, ₪{total_revenue:.2f} revenue")
            return summary
        ...
        try:
            ...
            for i in range(7):
                ...
                day_orders = [
                    order for order in all_orders 
                    if order.get('created_at') and 
                    order['created_at'].date() == day.date()
                ]
            ...
            self._logger.info(f"📈 WEEKLY TRENDS: {total_weekly_orders} orders, ₪{total_weekly_revenue:.2f}")
            return trends
    ...
    async def get_popular_products(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            product_stats = defaultdict(lambda: {'count': 0, 'total_quantity': 0, 'revenue': 0})
                ...
                [
                    {
                        ...
                        'avg_quantity_per_order': stats['total_quantity'] / stats['count']
                    }
            ...
            self._logger.info(f"📈 TOP PRODUCTS: {len(popular_products)} products analyzed")
    ...
    async def get_customer_insights(self) -> Dict[str, Any]:
        ...
        try:
            ...
            repeat_customers = len([cid for cid, orders in customer_orders.items() if len(orders) > 1])
            
            # Average orders per customer
            avg_orders_per_customer = len(all_orders) / active_customers if active_customers > 0 else 0
            ...
            insights = {
                ...
                'repeat_customer_rate': (repeat_customers / active_customers * 100) if active_customers > 0 else 0,
                'avg_orders_per_customer': avg_orders_per_customer,
                'avg_customer_lifetime_value': avg_customer_value,
                'top_customers': sorted(
                    [{'customer_id': cid, 'total_value': value, 'order_count': len(customer_orders[cid])}
            ...
            self._logger.info(f"📈 CUSTOMER INSIGHTS: {active_customers} active, {repeat_customers} repeat")
            return insights
    ...
    async def get_business_overview(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Status distribution
            status_distribution = Counter(order.get('status', 'pending') for order in all_orders)
            ...
            self._logger.info(f"📈 BUSINESS OVERVIEW: {total_lifetime_orders} total orders, ₪{total_lifetime_revenue:.2f}")
            return overview
