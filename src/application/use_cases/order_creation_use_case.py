...
from ..dtos.order_dtos import CreateOrderRequest, OrderCreationResponse, OrderItemInfo, OrderInfo
...
from ...infrastructure.utilities.exceptions import OrderCreationError, BusinessLogicError
from ...infrastructure.services.admin_notification_service import AdminNotificationService
...
class OrderCreationUseCase:
    ...
    def __init__(self, 
        ...
        self._logger.info(f"🏗️ ORDER USE CASE INITIALIZED")
        self._logger.info(f"  📁 Cart Repository: {type(self._cart_repository).__name__}")
        self._logger.info(f"  👤 Customer Repository: {type(self._customer_repository).__name__}")
        self._logger.info(f"  📋 Order Repository: {type(self._order_repository).__name__}")
        if self._admin_notification_service:
            self._logger.info(f"  📨 Admin Notification Service: {type(self._admin_notification_service).__name__}")
        else:
            self._logger.warning(f"  ⚠️ Admin Notification Service: NOT AVAILABLE")
    
    async def create_order(self, request: CreateOrderRequest) -> OrderCreationResponse:
        """Create order from user's cart"""
        self._logger.info(f"📝 ===== ORDER CREATION STARTED =====")
        ...
        try:
            ...
            # Get customer
            self._logger.info(f"👤 STEP A: Finding customer by telegram ID...")
            ...
            self._logger.info(f"✅ CUSTOMER FOUND: ID={customer.id.value}, Name={customer.full_name.value}, Admin={customer.is_admin}")
            
            # Get cart items
            self._logger.info(f"🛒 STEP B: Getting cart items...")
            ...
            for i, item in enumerate(cart_data['items']):
                self._logger.info(f"  📦 Item {i+1}: {item['quantity']}x {item['product_name']} - ₪{item['unit_price']:.2f}")
            
            # Calculate totals
            self._logger.info(f"💰 STEP C: Calculating totals...")
            subtotal = 0.0
            order_items = []
            ...
            delivery_method = request.delivery_method or cart_data.get('delivery_method') or 'pickup'
            ...
            if delivery_method == 'delivery' and not delivery_address:
                self._logger.error(f"❌ MISSING DELIVERY ADDRESS: User {request.telegram_id}")
                return OrderCreationResponse(
                    success=False,
            ...
            self._logger.info(f"💰 ORDER TOTALS CALCULATED:")
            ...
            # Create order
            self._logger.info(f"📋 STEP D: Creating order in database...")
            order_data = {
            ...
            self._logger.info(f"📋 ORDER DATA PREPARED: {len(order_data['items'])} items for customer {customer.id.value}")
            
            order = await self._order_repository.create_order(order_data)
            if not order:
                self._logger.error(f"💥 ORDER CREATION FAILED: Repository returned None")
                return OrderCreationResponse(
                    success=False,
                    error_message="Failed to create order. Please try again."
                )
            
            self._logger.info(f"✅ ORDER CREATED IN DATABASE:")
            ...
            self._logger.info(f"🧹 STEP E: Clearing cart...")
            await self._cart_repository.clear_cart(telegram_id)
            self._logger.info(f"✅ CART CLEARED")
            
            # Create order info response
            self._logger.info(f"📄 STEP F: Creating order info response...")
            ...
            self._logger.info(f"✅ ORDER INFO CREATED: Order #{order['order_number']}, Total=₪{total:.2f}")
            
            # Send admin notification if service is available
            self._logger.info(f"📨 STEP G: Handling admin notifications...")
            if self._admin_notification_service:
                self._logger.info(f"📨 ADMIN SERVICE AVAILABLE: Attempting to send notification...")
                try:
                    await self._admin_notification_service.notify_new_order(order_info)
                    self._logger.info(f"✅ ADMIN NOTIFICATION SENT: Order #{order['order_number']}")
                except Exception as e:
                    self._logger.error(f"💥 ADMIN NOTIFICATION FAILED: {e}", exc_info=True)
                    # Don't fail the order if notification fails
            else:
                self._logger.warning(f"⚠️ ADMIN NOTIFICATION SKIPPED: Service not available")
            
            self._logger.info(f"🎉 ===== ORDER CREATION COMPLETED =====")
            self._logger.info(f"🎉 Order #{order['order_number']} successfully created for {customer.full_name.value}")
