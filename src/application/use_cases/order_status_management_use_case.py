...
from ...infrastructure.services.customer_notification_service import CustomerNotificationService
from ...infrastructure.utilities.exceptions import BusinessLogicError, OrderNotFoundError
...
class OrderStatusManagementUseCase:
    ...
    async def update_order_status(self, order_id: int, new_status: str, admin_telegram_id: int) -> OrderInfo:
        """Update order status with validation and notifications"""
        self._logger.info(f"📝 STATUS UPDATE: Order {order_id} → {new_status} by Admin {admin_telegram_id}")
        ...
        try:
            ...
            # Update status
            success = await self._order_repository.update_order_status(order_id, new_status)
            if not success:
            ...
            # Get customer info
            customer = await self._customer_repository.find_by_id(updated_order['customer_id'])
            if not customer:
            ...
            self._logger.info(f"✅ STATUS UPDATED: Order #{order_info.order_number} {old_status} → {new_status}")
            ...
            self._logger.error(f"💥 STATUS UPDATE ERROR: Order {order_id}, {e}", exc_info=True)
            raise
    ...
    async def get_orders_by_status(self, status: str) -> List[OrderInfo]:
        ...
        try:
            ...
            filtered_orders = [order for order in all_orders if order.get('status') == status]
            
            order_infos = []
            for order_data in filtered_orders:
                customer = await self._customer_repository.find_by_id(order_data['customer_id'])
                if customer:
    ...
    async def _send_status_notifications(self, order_info: OrderInfo, old_status: str):
        ...
        try:
            ...
            if self._admin_notification_service:
                await self._admin_notification_service.notify_order_status_update(order_info, old_status)
                self._logger.info(f"📨 ADMIN STATUS NOTIFICATION SENT: Order #{order_info.order_number}")
            
            # Send customer notification
            if self._customer_notification_service:
                await self._customer_notification_service.notify_order_status_update(order_info, old_status)
                self._logger.info(f"📨 CUSTOMER STATUS NOTIFICATION SENT: Order #{order_info.order_number}")
